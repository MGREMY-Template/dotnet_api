...
public static partial class ResultExtensions
{
        ...
        int failureStatusCode)
    {
        ...
        (var isSuccess, var errors) = new TaskFactory().StartNew(() =>
        {
            return predicate(result.Value);
        }).Unwrap().GetAwaiter().GetResult();
    ...
    {
        return result.IsSuccess ?
            Result<TOut>.Success(new TaskFactory().StartNew(() =>
            {
                return mappingFunc(result.Value);
            }).Unwrap().GetAwaiter().GetResult(), result.StatusCode) :
            Result<TOut>.Failure(result.StatusCode, result.Messages);
    }
}
