...
public class UserClaimController : GenericController
{
    ...
    [HttpGet(nameof(Count)), Authorize(ClaimDefinition.IDENTITY_USERCLAIM_GETALL)]
    [ProducesResponseType(typeof(long), StatusCodes.Status200OK)]
    public async Task<IActionResult> Count(
        CancellationToken cancellationToken = default)
    {
        var result = await this._mediator.Send(new GetUserClaimCountQuery(), cancellationToken);
